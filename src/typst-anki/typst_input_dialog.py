...
class TypstInputDialog(QDialog):
    def __init__(self, parent=None, display_math=False, config={}):
        ...
        window_title = (
            "Input Typst math "
            + ("(inline)" if not display_math else "(block)")
            + "..."
        )
        ...
        self.mathml = QRadioButton(
            "MathML" if not display_math else "MathML (with `display: block`)"
        )
        self.typst_svg = QRadioButton(
            "Typst SVG" if not display_math else "Typst SVG (with `display: block`)"
        )

        self.radio_group = {"mathml": self.mathml, "svg": self.typst_svg}
        ...
        radio_layout.addWidget(self.mathml)
